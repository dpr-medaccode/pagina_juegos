...
export default function DetallePlataformaVista() {
  ...
  const [loadingPlataforma, setLoadingPlataforma] = useState(true);
  ...
  useEffect(() => {
    async function loadTag() {
      ...
      const p = await Plataforma.new_from_id(id);
      setPlataforma(p);
  ...
  useEffect(() => {
    ...
    async function loadJuegos() {
      ...
      const data = await plataforma.juegos(pagina, 12);
      setJuegos(data);
  ...
  return (
    ...
    <motion.div className="max-w-6xl mx-auto px-6 py-10 space-y-8 text-zinc-100"
      ...
      {plataforma.image_background && plataforma.games_count !== 0 && (
        <motion.img
