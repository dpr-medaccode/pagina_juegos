...
import { loadPublisherPorId, loadPublisherJuegos, clearPublisher } from "../../features/publishers/idPublishersSlice";
...
export default function DetallePublisherVista() {
  ...
  const {
    ...
    error,
    pagina: paginaRedux,
  } = useSelector(state => state.idPublishers);


  useEffect(() => {

    dispatch(loadPublisherPorId(id));
  ...
  useEffect(() => {
    ...
    dispatch(loadPublisherJuegos({ id, pagina, cantidad: 12 }));

  }, [dispatch, publisher, id, pagina])
